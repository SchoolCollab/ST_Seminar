...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Setup time | Not retrospectively timed. The repeatable setup path is documented instead: import `EShop_OpenApi.yaml`, create `Local`, set the Base URL, seed `userEmail/userPassword/adminEmail/adminPassword`, run login once to populate `bearerToken`, then import/run the saved suite checkpoint. | Not retrospectively timed. Recorded setup facts: hosted Gemini/Google free-plan provider, all Apidog generation categories selected, `{{bearerToken}}` supplied as the credential variable, and generation frozen at two endpoint sets because the second endpoint was delayed by free-plan bandwidth/API limits. | Not retrospectively timed. The landed work required backend test-mode hooks, provider states, three consumer API-client extractions, Jest/Pact setup for web/admin/mobile, and provider verification across 51 interactions. |
| Run time   | Latest classified full-suite report: `apidog-reports-2026-08-10-15-59-56.html`, 263 HTTP requests, 109 failed requests, 282 assertions, 115 failed assertions, 58.56% passed. The report exposes per-request `Time Consuming` values; summed HTTP time is `29.473s` across 263 requests. This is not a green CI baseline. | `2.20s` for `PUT /api/users/me`: 25 requests, 9 passed, 16 failed, 35 assertions, 23 failed. `1.66s` for `GET /api/products/{id}`: 22 requests, 3 passed, 19 failed, 18 assertions, 18 failed. Combined executed scope: 47 requests, 12 passed, 35 failed, 53 assertions, 41 failed, `3.86s` report runtime. | Full `run_tests.sh` wall-clock duration was not captured in a reusable log. The verified result is 51 interactions, 46/51 provider verification across `eshop-web` 14/17, `eshop-admin` 20/21, and `eshop-mobile` 12/13. |
| Flake rate | Not measured as a stabilized repeated-run baseline. Multiple setup/debug runs exist, but they were not comparable N=3/N=5 flake trials because the suite structure and environment reset strategy changed between runs. | Not measured. Both AI endpoint reports were one-run evidence, useful for oracle review but not a repeated-run stability claim. | Provider verification was deliberately rerun during Pact build work before accepting unexpected failures; the documented baseline stabilized at five named failures. A final N=3 `run_tests.sh` wall-clock flake trial was not captured, so report the stability evidence qualitatively rather than inventing a rate. |

Interpretation: M5 is deliberately honest rather than artificially complete.
Only Apidog report runtimes and AI endpoint runtimes are directly recorded as
durations. Setup time and final flake rate were not measured under controlled
conditions, so they are documented as scope caveats instead of estimated values.
