...
actual)" placeholder in `Material/Document/Apidog/EShop_Apidog_TestCases.md` for
Track A's endpoints, so Apidog case building is mechanical.

Cross-refs used, all traced back to `server.js` at the line ranges cited:
`server.js` (canonical), `Material/Document/SUT-Reference/EShop_Defect.md`,
`EShop_OpenApi.yaml`, `Material/Document/Apidog/EShop_Apidog_Steps.md` Steps
6/6a and 9, `Material/Document/Apidog/EShop_Apidog_TestCases.md`.
...
Step 3 of `Material/Document/Apidog/EShop_Apidog_Steps.md` is the closure gate.
Actions:
...
`Material/Document/Apidog/EShop_Apidog_TestCases.md` intro §"(defect demo) tag"
— the expected 200 is the wrong outcome, but it is the observed outcome, and
asserting on the observed outcome is what makes the case fail if the defect is
ever fixed.
...
`role` now reads `"admin"`. This is standing evidence for SEC-06 only. It is not
the current admin-token setup path: the current suite logs in with the seeded
`admin@eshop.com` account and overwrites `bearerToken` before admin cases.
...
Pact `eachLike` gap noted in `Material/Document/Pact/EShop_Pact_Plan.md` §6).
The explicit `$.password Exists` assertion is what makes SEC-01 catchable.
...
`Material/Document/Apidog/EShop_Apidog_Steps.md` Step 9 in tightened form, with
every request body, variable extraction, and assertion pinned to what
`server.js` returns.
...
5. Configure assertions per the "Assertions" column using the shorthand from
   `Material/Document/Apidog/EShop_Apidog_TestCases.md` §"Assertion and
   processor shorthand".
...
the defect-demo tag and the `Material/Document/SUT-Reference/EShop_Defect.md`
cross-reference. See the diff on the doc for the exact edit.
