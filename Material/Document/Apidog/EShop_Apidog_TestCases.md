...
`EShop_OpenApi.yaml`. `Material/Document/Apidog/EShop_Apidog_Steps.md` (Step
6/6a) explains _how_ to configure a case in Apidog once, in full detail, using
`POST /api/cart` as the worked example; this document is _what values to use_
for every other endpoint, and — as of this revision — _exactly which assertions
and processors each case needs_, so no case is left to interpretation when you
build it.
...
auth enforcement (see `Material/Document/SUT-Reference/EShop_Defect.md`). For
these, the expected outcome is often a **200 that shouldn't happen** — the case
exists to catch and document the defect. Every such case cites the relevant
entry in `Material/Document/SUT-Reference/EShop_Defect.md`.
...
| ------------------------ | ---------------------- | -------------------------------------------------------------------------------------- | ---------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Success                  | Positive               | `{ "name": "New Tester", "email": "tester.new@example.com", "password": "Test1234!" }` | `Status=200`; `RV@200` | `Post: newUserId ← $.id` (capture for any later cleanup) |                                                                                                                              |
...
| Duplicate email accepted | Negative (defect demo) | Same, `email: "test@eshop.com"`                                                        | `Status=200`           | `None`                                                   | Uses the seeded regular user's email. No unique constraint on email — see `Material/Document/SUT-Reference/EShop_Defect.md`. |
...
| Case        | Category | Body                                                                                          | Assertions             | Processors                             | Notes                                                                                           |
| ----------- | -------- | --------------------------------------------------------------------------------------------- | ---------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------------- |
| Success     | Positive | `email`, `{{resetToken}}` from the forgot-password case, `newPassword: "{{newUserPassword}}"` | `Status=200`; `RV@200` | `Post: userPassword ← newUserPassword` | Chain after the forgot-password success case, or paste the token manually for a standalone run. |
...
| Role injection           | Security (defect demo) | `{ "role": "admin" }`                                                                                              | `{{bearerToken}}` | `Status=200`           | `None`     | SEC-06 defect evidence only. Do not use this to provision admin access; the current suite uses the seeded `admin@eshop.com` login and overwrites `bearerToken` before admin cases. See `Material/Document/SUT-Reference/EShop_Defect.md`. |
...
| Case                         | Category               | Body                                                            | Auth                                  | Assertions                               | Processors | Notes                                                                                                               |
| ---------------------------- | ---------------------- | --------------------------------------------------------------- | ------------------------------------- | ---------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------- |
| All valid                    | Positive               | Two valid products (`allValid` example in `EShop_OpenApi.yaml`) | `{{bearerToken}}` (after admin login) | `Status=200`; `RV@200`                   | `None`     | Reported `inserted` count may be wrong — async callback bug, see `Material/Document/SUT-Reference/EShop_Defect.md`. |
| Partial invalid, no rollback | Negative (defect demo) | One row missing `name` (`partiallyInvalid` example)             | `{{bearerToken}}` (after admin login) | `Status=200`; `Body:$.inserted Equals 1` | `None`     | FR-16 requires all-or-nothing. See `Material/Document/SUT-Reference/EShop_Defect.md`.                               |
...
Fully worked in `Material/Document/Apidog/EShop_Apidog_Steps.md`, Step 6/6a —
repeated here for a complete per-endpoint reference, values identical.
...
| Quantity = 0 accepted           | Boundary (defect demo) | Same, `quantity: 0`                                                     | `{{bearerToken}}` | `Status=200`; `Body:$.message Equals "Added to cart"`                        | `None`     | `server.js:322–327` pushes `req.body` verbatim with no validation — see `Material/Document/SUT-Reference/EShop_Defect.md` "accepts any JSON shape." |
...
| Case                | Category               | Body                                                                                                                                            | Auth                                  | Assertions             | Processors                                                   | Notes                                                  |
| ------------------- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | ---------------------- | ------------------------------------------------------------ | ------------------------------------------------------ |
| Success             | Positive               | `{ "code": "SAVE10", "type": "percent", "discount_value": 10, "min_order_amount": 200000, "expired_at": "2027-01-31", "max_uses_per_user": 1 }` | `{{bearerToken}}` (after admin login) | `Status=200`; `RV@200` | `Post: createdCouponId ← $.id` (feeds the DELETE case below) |                                                        |
...
| Duplicate code      | Negative               | Reuse an existing `code`                                                                                                                        | `{{bearerToken}}` (after admin login) | `Status=500`           | `None`                                                       | DB unique-constraint error.                            |

### `DELETE /api/admin/coupons/{id}`

| Case                | Category               | Auth                                               | Assertions                             | Processors | Notes                                                  |
| ------------------- | ---------------------- | -------------------------------------------------- | -------------------------------------- | ---------- | ------------------------------------------------------ |
| Success             | Positive               | `{{bearerToken}}` (after admin login)              | `Status=200`; `RV@200`                 | `None`     | Use `{{createdCouponId}}`.                             |
...
| Not found           | Negative               | `{{bearerToken}}` (after admin login), `id=999999` | `Status=` **(verify — record actual)** | `None`     |                                                        |

---

## Admin

The live Apidog checkpoint no longer uses an `adminToken` variable. Admin
success cases use `{{bearerToken}}` after the Admin section's login producer has
logged in as `admin@eshop.com` / `Admin123!` and overwritten the previous
regular-user JWT. Rows marked "regular user" still mean the same
`{{bearerToken}}` variable, but immediately after a regular-user login.

### `GET /api/admin/users`

| Case                | Category               | Auth                                  | Assertions                                             | Processors | Notes                                                                                                         |
| ------------------- | ---------------------- | ------------------------------------- | ------------------------------------------------------ | ---------- | ------------------------------------------------------------------------------------------------------------- |
| Success             | Positive               | `{{bearerToken}}` (after admin login) | `Status=200`; `RV@200`; `Body:$[0].password NotExists` | `None`     | Contrast with `GET /api/users/me`, which leaks `password` — worth noting the inconsistency in the User Guide. |
| No admin-role check | Security (defect demo) | `{{bearerToken}}` (regular user)      | `Status=200`                                           | `None`     | See `Material/Document/SUT-Reference/EShop_Defect.md`.                                                        |
| No auth             | Security               | No Auth override                      | `Status=401`                                           | `None`     |                                                                                                               |
...
| Case                     | Category               | Path                                                  | Auth                                  | Assertions             | Processors | Notes                                                                                                                                                                |
| ------------------------ | ---------------------- | ----------------------------------------------------- | ------------------------------------- | ---------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Success                  | Positive               | A disposable throwaway user's id (register one first) | `{{bearerToken}}` (after admin login) | `Status=200`; `RV@200` | `None`     | Never run against the seeded regular user (`test@eshop.com`) or your main admin account.                                                                             |
| Self-delete allowed      | Security (defect demo) | The **admin account's own id**                        | `{{bearerToken}}` (after admin login) | `Status=200`           | `None`     | **Caution:** use a second, disposable admin account for this case — it will actually delete the account used. See `Material/Document/SUT-Reference/EShop_Defect.md`. |
...
| Nonexistent id still 200 | Negative (defect demo) | `id=999999`                                           | `{{bearerToken}}` (after admin login) | `Status=200`           | `None`     | See `Material/Document/SUT-Reference/EShop_Defect.md`.                                                                                                               |

### `GET /api/admin/orders`

| Case                | Category               | Auth                                  | Assertions                                           | Processors | Notes                                                  |
| ------------------- | ---------------------- | ------------------------------------- | ---------------------------------------------------- | ---------- | ------------------------------------------------------ |
| Success             | Positive               | `{{bearerToken}}` (after admin login) | `Status=200`; `RV@200`; `Body:$[0].user_name Exists` | `None`     |                                                        |
| No admin-role check | Security (defect demo) | `{{bearerToken}}` (regular user)      | `Status=200`                                         | `None`     | See `Material/Document/SUT-Reference/EShop_Defect.md`. |
...
| --------------------------------------- | ---------------------- | --------------------------- | --------------------- | ------------------------------------- | ---------------------- | ---------- | ---------------------------------------------------------------------------------------------------- |
| Valid transition                        | Positive               | `{ "status": "confirmed" }` | `pending`             | `{{bearerToken}}` (after admin login) | `Status=200`; `RV@200` | `None`     |                                                                                                      |
| Illegal transition (correctly rejected) | Negative               | `{ "status": "delivered" }` | `pending`             | `{{bearerToken}}` (after admin login) | `Status=400`           | `None`     | This guard works correctly.                                                                          |
| Bug: canceled → delivered accepted      | Negative (defect demo) | `{ "status": "delivered" }` | `canceled`            | `{{bearerToken}}` (after admin login) | `Status=200`           | `None`     | Should be rejected — both should be terminal. See `Material/Document/SUT-Reference/EShop_Defect.md`. |
