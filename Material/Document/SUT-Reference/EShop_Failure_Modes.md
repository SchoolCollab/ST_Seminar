...
### FM-10 — Apidog CLI `--project` can cause a misleading project-permission failure

**What happened.** The GitHub Actions Apidog workflow initially called
`apidog run` with a separate `--project` argument in addition to
`--test-suite`, `-e`, and the access token. The token was present and Apidog's
UI showed it had been used, but the CI run failed with `403010 No project guest
privilege`.

**Where it showed up.** In `.github/workflows/apidog-suite.yml` while bringing
the `EShop — Full Regression` suite into CI/CD. The failure happened before the
suite actually ran, so no useful request/assertion report was produced.

**Why it's misleading.** The error text points at account/project access, so it
looks like the user generated the token from the wrong Apidog account or lacks
project membership. In this case the account did own the project and the token
was valid; the extra CLI argument was the difference from Apidog's own generated
CI/CD command template.

**Root cause.** The working Apidog-generated command identifies the suite and
...
Adding a separate `--project` argument was unnecessary and changed the CLI
authorization path enough to trigger the misleading `403010` error.

**Resolution.** Removed `--project` and `APIDOG_PROJECT_ID` from the workflow.
The committed workflow now follows the generated template shape, keeps
`APIDOG_ACCESS_TOKEN` as the only secret, and exposes only
`APIDOG_TEST_SUITE_ID` and `APIDOG_ENVIRONMENT_ID` as future-change variables.
The next CI run reached the real suite execution step and exported an HTML
report artifact.

**Lesson.** For Apidog CI/CD, start from the command generated by the suite's own
CI/CD tab and make the smallest possible adaptations. If a valid token produces
`403010`, compare the exact CLI command against Apidog's generated template
before assuming the GitHub secret or account membership is wrong.
